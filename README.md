...
- Durante o desenvolvimento do sistema, encontrei alguns problemas como a aplicação de debounce à função que lê o botão e muda o estado da porta, pois tentei fazer isso por meio de uma interrupção no acionamento do botão, mas ocorriam alguns erros como o acionamento do alerta da porta aberta, mesmo sem que o botão esteja pressionado, que me fez tirar a interrupção do programa e usar apenas uma função "normal" na execução; e impressão contínua das mensagens de alerta, que foi resolvido ao adicionar variáveis para checar o estado anterior de cada tipo de alarme, imprimindo uma mensagem apenas quando o estado atual (se for True) for diferente do passado (False). Por fim, um problema que não consegui resolver foi a retirada das funções bloqueantes time.sleep_ms() no while True: e para imprimir a mensagem de sistema normalizado (este foi aplicado para passar no teste 3, pois aparentemente a mensagem estava sendo impressa fora da "janela de observação" e a avaliação automática não a recebia), então as mantive para poder enviar o projeto sem erros no GitHub Actions.
